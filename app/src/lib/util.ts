import {captureException} from "@sentry/react"

/* eslint-disable-next-line */
export const EMAIL_REGEX = /(?:[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*|"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])/


export function get_lang() {
    // @ts-ignore
    return navigator.userLanguage || (navigator.languages && navigator.languages.length && navigator.languages[0]) || navigator.language || navigator.browserLanguage || navigator.systemLanguage || "en"
}

export function slugify(s: string): string {
    return s.toLowerCase().replace(/ /g, "-").replace(/%/g, "")
}


export function parameterize_url(url: string, params: {[key: string]: string}): string {
    const args = Object.keys(params).map(k => `${k}=${encodeURIComponent(params[k])}`).join("&")
    return url + "?" + args
}


export function timeout<T>(ms: number, promise: Promise<T>): Promise<T> {
    let id: NodeJS.Timeout | null = null
    const t = new Promise<T>((resolve, reject) => {
        id = setTimeout(() => {
            reject(new Error("Timeout"))
        }, ms)
    })
    return Promise.race([promise, t])
        .finally(() => {
            if (id !== null) {
                clearTimeout(id)
            }
        })
}

/** Execute an async/function / promise, but log to console & sentry any errors
 *
 * @param action Async function or promise to execute
 */
export function safe_exec(action: () => Promise<any>) {
    action().catch(e => {
        captureException(e)
        console.log(e)
    })
}